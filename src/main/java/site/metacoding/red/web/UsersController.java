...
public class UsersController {
	...
	public String join(JoinDto joinDto) {
		usersServivce.회원가입(joinDto);
		return "redirect:/loginForm";
	}
	
	@PostMapping("/login")
	public @ResponseBody String join(LoginDto loginDto) {
		Users principal = usersServivce.로그인(loginDto);
		if(principal == null) {
	...
	public String updateForm(@PathVariable Integer id, Model model) {
		Users usersPS = usersServivce.회원정보보기(id);
	...
	public String update(@PathVariable Integer id, UpdateDto updateDto) {
		usersServivce.회원수정(id, updateDto);
		return "redirect:/users/"+id;
	}
	
	@DeleteMapping("/users/{id}")
	public @ResponseBody String delete(@PathVariable Integer id) {
		usersServivce.회원탈퇴(id);
